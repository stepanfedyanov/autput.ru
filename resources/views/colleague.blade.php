...
<html lang="ru">
  ...
  <body>
    ...
    <section class="colleague">
      <div class="container">
        <div class="colleague__wrapper">
          @if ($colleague->cover)
          <div class="cover" style="background-image: url('{{$colleague->cover}}')"></div>
          @endif
          <div class="avatar">
            <div class="avatar__wrapper">
              @if ($colleague->avatar)
              <div class="avatar__img" style="background-image: url('{{$colleague->avatar}}')"></div>
              @endif
              <div class="avatar__title">
                <p class="text__main black left">{{$colleague->firstName}} {{$colleague->lastName}}</p>
                <p class="text__small gray left">{{$colleague->jobTitle}}</p>
              </div>
            </div>
            <div class="avatar__text">
              @if ($colleague->socialLink AND $colleague->socialLinkName)
              <a class="link__main black right" href="{{$colleague->socialLink}}">{{$colleague->socialLinkName}}</a>
              @endif
              <p class="text__small black right">
                В&nbsp;портфолио {{count($works)}}
                @if (count($works) === 1)
                работа
                @elseif (count($works) > 1 AND count($works) < 5)
                работы
                @else
                работ
                @endif
